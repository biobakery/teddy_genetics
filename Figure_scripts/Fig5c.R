...
position <- read.csv("./data/genetics.bed/filter/teddy-out8_map.map",header=F,sep="\t") %>%
  rename(chr=1, snp=2, morgans=3, bp=4)


### Map rsID to corresponding gene
map <- fread("./data/map.txt")
map <- map %>% select(V4,V8,V9)
map <- map %>% rename(snp = V4, symbol = V8, gene_id = V9)
map <- map %>% filter(!duplicated(snp))

### Filter interaction results
inter <- select_species %>% filter(grepl(":",Predictor))
inter <- inter %>% rename(snp = rsid)
inter <- inter %>% mutate(log10P = -log10(P))

### Select significant results for further annotation
sig <- inter %>% filter(P < 5e-8)
species <- names(table(sig$Species))
inter_sepcies <- inter %>% filter(Species %in% species)

### Merge with SNP map and calculate cumulative base pair positions
man <- read.csv("./data/genetics.bed/filter/teddy-out8_map.map",
                header=F,
                sep="\t") %>%
  rename(chr=1, snp=2, morgans=3, bp=4) %>%
  arrange(chr, bp) %>%
  mutate(bp = cumsum(as.numeric(bp))) %>%
  merge(., inter_sepcies, by="snp")

### Load taxonomy information for futher annotation
...
### annotate most significant SNPs for each species.
...
manhplot <- ggplot(man, aes(x=bp, y=log10P)) +
  ...
  #facet_wrap(~Species, scales="intered", nrow=length(species)) +
  ...
  theme_bw() +ggtitle("Toddler_intered")+
  theme( 
